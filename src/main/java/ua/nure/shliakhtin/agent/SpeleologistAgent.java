...
public class SpeleologistAgent extends Agent {
    ...
    protected void setup() {
        log.log(Level.SEVERE, "Speleologist: Hello! The speleologist agent " + getAID().getName() + " is ready.");
    ...
    protected void takeDown() {
        log.log(Level.SEVERE, "Speleologist: The speleologist agent " + getAID().getName() + " terminating.");
    }
    ...
    private class WumpusWorldFinder extends Behaviour {
        ...
        @Override
        public void action() {
            log.log(Level.SEVERE, "Speleologist: Start finding a Wumpus world");
            ...
            try {
                ...
                if (result != null && result.length > 0) {
                    ...
                    log.log(Level.SEVERE, "Speleologist: Unfortunately, a Wumpus world hasn't been found :(.");
                    block();
                }
                isDone = true;
        ...
        public void action() {
            switch (step) {
                case 0:
                    ...
                    cfp.setContent(AppConstant.GO_INSIDE);
                    cfp.setConversationId(AppConstant.WUMPUS_WORLD_DIGGER_CONVERSATION_ID);
                    cfp.setReplyWith("cfp" + System.currentTimeMillis());
                    myAgent.send(cfp);
                    mt = MessageTemplate.and(MessageTemplate.MatchConversationId(AppConstant.WUMPUS_WORLD_DIGGER_CONVERSATION_ID),
                ...
                case 1:
                    ...
                    if (reply != null) {
                        if (reply.getPerformative() == ACLMessage.CONFIRM) {
                            log.log(Level.SEVERE, "Speleologist: Found a Wumpus World!");
        ...
        public void action() {
            switch (step) {
                case 0:
                    ...
                    inform.setContent(AppConstant.ADVICE_PROPOSAL);
                    inform.setConversationId(AppConstant.NAVIGATOR_DIGGER_CONVERSATION_ID);
                    inform.setReplyWith("inform" + System.currentTimeMillis());
                    log.log(Level.SEVERE, "Speleologist: " + AppConstant.ADVICE_PROPOSAL);
                    myAgent.send(inform);
                    mt = MessageTemplate.and(MessageTemplate.MatchConversationId(AppConstant.NAVIGATOR_DIGGER_CONVERSATION_ID),
                    ...
                    if (reply != null) {
                        if (reply.getPerformative() == ACLMessage.REQUEST) {
                            if (parseNavigatorMessageRequest(reply.getContent())) {
                                ...
                                req.setContent(AppConstant.GAME_INFORMATION);
                                req.setConversationId(AppConstant.WUMPUS_WORLD_DIGGER_CONVERSATION_ID);
                                ...
                                log.log(Level.SEVERE, "Speleologist: There is an unknown command!");
                        }
                    } else {
                ...
                case 2:
                    ...
                    if (reply != null) {
                        if (reply.getPerformative() == ACLMessage.INFORM) {
                            ...
                            mes.setContent(AppConstant.INFORMATION_PROPOSAL_SPELEOLOGIST + rep);
                            mes.setConversationId(AppConstant.NAVIGATOR_DIGGER_CONVERSATION_ID);
                            mes.setReplyWith("mes" + System.currentTimeMillis());
                            log.log(Level.SEVERE, "Speleologist: " + AppConstant.INFORMATION_PROPOSAL_SPELEOLOGIST + rep);
                            myAgent.send(mes);
                            mt = MessageTemplate.and(MessageTemplate.MatchConversationId(AppConstant.NAVIGATOR_DIGGER_CONVERSATION_ID),
                        ...
                        if (reply.getPerformative() == ACLMessage.PROPOSE) {
                            ...
                            switch (action) {
                                case Commands.TURN_LEFT:
                                    content = AppConstant.SPELEOLOGIST_TURN_LEFT;
                                    break;
                                case Commands.TURN_RIGHT:
                                    content = AppConstant.SPELEOLOGIST_TURN_RIGHT;
                                    break;
                                case Commands.MOVE_FORWARD:
                                    content = AppConstant.SPELEOLOGIST_MOVE_FORWARD;
                                    break;
                                case Commands.GRAB:
                                    content = AppConstant.SPELEOLOGIST_GRAB;
                                    break;
                                case Commands.SHOOT:
                                    content = AppConstant.SPELEOLOGIST_SHOOT;
                                    break;
                                case Commands.CLIMB:
                                    content = AppConstant.SPELEOLOGIST_CLIMB;
                                    break;
                                default:
                                    log.log(Level.SEVERE, "Speleologist: There is no right action!");
                                    break;
                            }
                            ...
                            prop.setConversationId(AppConstant.WUMPUS_WORLD_DIGGER_CONVERSATION_ID);
                ...
                case 4:
                    ...
                    if (reply != null) {
                        ...
                        switch (answer) {
                            case AppConstant.FAIL_MESSAGE:
                                log.log(Level.SEVERE, "Speleologist: You failed!");
                                step++;
                                doDelete();
                                break;
                            case AppConstant.OK_MESSAGE:
                                log.log(Level.SEVERE, "Speleologist: Wumpus world answers OK.");
                                step = 0;
                                break;
                            case AppConstant.WIN_MESSAGE:
                                log.log(Level.SEVERE, "Speleologist: The speleologist survived and won!");
                                step++;
                                doDelete();
                                break;
                        }
